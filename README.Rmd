...
This is looking better, but we want to remove any unnecessary elements that take away from the data display. The `ggplot2::theme_minimal()` function removes a lot of the extra graph elements to give a cleaner look.
...
```{r create-index.html, eval=FALSE}
knitr::knit(input = "README.Rmd", output = "docs/index.html")
```
